...
def transformer_model_fn(features, labels, mode, params):
    with tf.variable_scope('NmtModel') as var_scope:
            ...
            for i, device in enumerate(devices):
                ...
                with tf.variable_scope( tf.get_variable_scope(), reuse=True if i > 0 else None):
                    with tf.device(device):
                        context_shard = None
                        if use_script_embedding or use_word_embedding: context_shard = context_shards[i]
        ...
        if mode == tf.estimator.ModeKeys.EVAL:
            ...
            if use_script_embedding or use_word_embedding: tf.add_to_collection("contexts", contexts)
...
def GenerateSignature():
    ...
    if use_word_script_embedding or use_script_embedding:
