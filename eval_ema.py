...
class SaveEvaluationPredictionHook(tf.train.SessionRunHook):
  def __init__(self, output_file, dev_src, dev_trg, trg_vocab_file, acc_log):
    ...
    self.devsrcs = [line.strip() for line in open(dev_src).readlines()]
    self.devtrgs = [line.strip() for line in open(dev_trg).readlines()]
    self.dev_index = -1
...
def main(_):
    ...
    print("eval_model_dir:", eval_model_dir)
    ...
    if False:
        ...
        print("ckpt:", ckpt)
        # if params['ema_decay'] > 0.0:
        #     # cvt(model_dir.rstrip('\/')+'/'+ckpt.replace('.index',''),eval_model_dir+"/ema")
        #     cvt(model_dir.rstrip('\/')+'/'+ckpt.replace('.index',''),eval_model_dir+"")
        transformer.evaluate(eval_input_fn, hooks=eval_hooks)
