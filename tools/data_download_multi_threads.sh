...
${ODPS_BIN} -e "
tunnel download ${char_train_table} ${work_dir}/char_train_table -fd '\t' -threads 10;
"

# split the table into train.src & train.trg
awk -F "\t" '{print $1}' ${work_dir}/char_train_table/* > ${work_dir}/train.src 
awk -F "\t" '{print $2}' ${work_dir}/char_train_table/* > ${work_dir}/train.trg


# download char_vocab_table
${ODPS_BIN} -e "
tunnel download ${char_vocab_table} ${work_dir}/train.vocab -fd '\t';
"
